...
def wait_for_node():
    ...
    while not w3 or not w3.is_connected():
        try:
            w3 = Web3(Web3.HTTPProvider('http://localhost:8545', request_kwargs={'timeout': 30}))
            if w3.is_connected():
        ...
        if attempts >= max_attempts:
            raise Exception("Node failed to start after maximum attempts")
